...
```bash
# 1. Add SSH key (passphrase-protected — required before any ssh/scp)
ssh-add ~/.ssh/id_ed25519

# 2. Transfer source (fathom files needed for v25+)
scp engine/vN_engine.c engine/Makefile \
...
python3 tournament_v22.py --engine1 ./vTest_engine --engine2 ./v25_baseline \
    --openings 25 --movetime 100 --seed 42 2>&1 | tail -8
```

**Keep if**: perft = 4865609 AND tournament ≥ 48%. Revert immediately otherwise.
**Baseline binary**: `engine/v25_baseline` (copy before adding features: `cp vN_engine vN_baseline`)
...
**Workflow**: cp vN_engine.c → vTest_engine.c → test → cp vTest_engine.c → v(N+1)_engine.c
...
|---------|--------|-------|
| PV LMR modifier (reduce−1 at PV) | 53% | Reverted v25 session |
| Recapture extension | 48% | Reverted v26 session |
...
| 2-ply continuation history | 47.8% | Reverted v26 (old) session |
...
|---------|--------------|
| v13 | C rewrite, BBC, UCI, ~1.3M nps |
| v14 | Time management, -march=native, 64MB TT, reverse futility |
| v15 | Rook bonuses, knight outpost, backward pawns, tapered PSTs |
| v16 | IID, aspiration widening, score instability detection |
| v17 | SEE, king attack count, adaptive null move R, countermove, history malus+gravity, tempo, bad bishop |
| v18 | Lazy SMP, pawn hash, Probcut, cont history, capture history, TT pre-try, Singular Extensions |
| v19 | Razoring, history-adjusted LMR, LMP, king tropism, passed pawn proximity, mop-up eval, threat detection |
| v20 | Texel-tuned PSTs (Lichess DB, 1M positions, K=844, 28 epochs) |
| v21 | Pondering (TT collision fix, ponder move validation, early-finish loop) |
| v22 | Clock-differential time management (1.08–1.25× budget when ahead) |
| v23 | Mate Distance Pruning, ETT cache (128K), endgame scaling, safe mobility |
...
# Compile tuner
make v26_tuner   # -DTUNER flag disables correction history and pondering
